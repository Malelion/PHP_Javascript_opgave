...
<!-- CO2 Slut -->
<div class="luftkvalitet" id="luftkval">
...
<g id="Layer_4">
    ...
    <div class="luftfugt" id="luftfugt">
...
<!-- Luftfugtighed Slut -->
        ...
        function loadDoc() {
            var fugtRequest = new XMLHttpRequest(); 
            fugtRequest.onreadystatechange = function() {
                if (this.readyState == 4 && this.status == 200) {
                    var myObj = JSON.parse(this.responseText);
                    handler(myObj);
                    document.getElementById("luftfugt").innerHTML = myObj;
                }
            };
            fugtRequest.open("GET", "nygrafData.php?s=luftfugtighed&sys=IdaErin", true);
            
            fugtRequest.send();
        
            var kvalRequest = new XMLHttpRequest(); 
            kvalRequest.onreadystatechange = function()
            {
                if (this.readyState == 4 && this.status == 200){
                    var myObj = JSON.parse(this.responseText);
                    handler(myObj);
                    document.getElementById("luftkval").innerHTML = myObj;
                }
            };
            kvalRequest.open("GET", "nygrafData.php?s=CO2&sys=luftkvalitet", true);
            ...
            tempRequest.open("GET", "nygrafData.php?s=lufttemperatur&sys=IdaErin", true);
            
            tempRequest.send();
        
            var jordRequest = new XMLHttpRequest(); 
            jordRequest.onreadystatechange = function()
            {
                if (this.readyState == 4 && this.status == 200){
                    var myObj = JSON.parse(this.responseText);
                    handler(myObj);
                    document.getElementById("jord").innerHTML = myObj.vaerdi;
                }
            };
            jordRequest.open("GET", "nygrafData.php?s=jordfugtighed&sys=IdaErin", true);
           ...
           // var tidspunkt=  inputData[0].obstime;
            var vaerdi = inputData[0].value;
            }
